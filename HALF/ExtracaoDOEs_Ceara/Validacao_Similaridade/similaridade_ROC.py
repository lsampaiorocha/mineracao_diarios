...
def plotar_curva_roc(fpr, tpr, thresholds, melhor_limiar_idx, melhor_limiar, roc_auc):
    ...
    plt.legend()
    plt.savefig('curva_roc_1000final.png', dpi=300)
    plt.close()
...
if __name__ == "__main__":
    # Carregar dados
    df, numeracao, pares, rotulos = carregar_dados("Validação de Assuntos.xlsx")
    ...
    print("\nExemplo de Pares:")
    for i in np.random.choice(len(pares), 1, replace=False):
