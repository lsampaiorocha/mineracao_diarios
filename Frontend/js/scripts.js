...
function executar(e){
    ...
    e.preventDefault();
    if(c == 1){
        orgaoInput = document.querySelector("#orgao");
        assuntoInput = document.querySelector("#assunto");
        ...
        orgao = orgaoInput ? orgaoInput.value.toLowerCase() : '';
        /*Aqui a contante orgao é normalizada para não possuir acentos dentro do orgaoSem */
        orgaoSem = orgao.normalize("NFD").replace(/[\u0300-\u036f]/g, "")
        /*Abaixo, ocorre a mesma coisa com o assunto*/
        assunto = assuntoInput ? assuntoInput.value.toLowerCase() : '';
        assuntoSem = assunto.normalize("NFD").replace(/[\u0300-\u036f]/g, "")
        /*Adiciona as constantes data de inicio e  fim*/
        datai = new Date(document.querySelector("#datai").value);
        dataf = new Date(document.querySelector("#dataf").value);
        /*Nome no caso palavra-chave passa pelo mesmo processo do orgão e do assunto */
        nome = nomeInput ? document.querySelector("#nome").value.toLowerCase() : '';
        nomeSem = nome.normalize("NFD").replace(/[\u0300-\u036f]/g, "")
            ...
            .then(arquivos => {
                ...
                if(c == 1){
                    const resultados = arquivos.filter(arquivo => {
                        /*Verifica se todos os elemetos da busca estão em branco*/
                    ...
                    resultados.forEach(resultado => {   
                        ...
                        let destaque = String(resultado.DESTAQUE).replace(/\n/g, "<br>")
                        item.innerHTML = `
                ...
                if(c == 0){
                    arquivos.forEach(arquivo =>{
                        if(listaAssuntos.includes(arquivo.assunto)){
                            aux = listaAssuntos.indexOf(arquivo.assunto)
                            numeroAssunto[aux] += 1
                        }
                        else{
                            listaAssuntos.push(arquivo.assunto)
                            numeroAssunto.push(1)
                        }
                        if(listaDatas.includes(arquivo.DATA)){
                            aux = listaDatas.indexOf(arquivo.DATA)
                            numeroData[aux] += 1
                        }
                        else{
                            listaDatas.push(arquivo.DATA)
                            numeroData.push(1)
                        }
                    })
                    m = Math.max(...numeroAssunto)
                    n = numeroAssunto.indexOf(m)
                    assuntoMais.innerHTML = listaAssuntos[n] + "(" + m + ")"
                    m = Math.max(...numeroData)
                    n = numeroData.indexOf(m)
                    dataMais.innerHTML = listaDatas[n].split("-").reverse().join("-") + "(" + m + ")"
                }    
            })
